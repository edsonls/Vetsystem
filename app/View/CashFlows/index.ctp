<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><?php echo __('Cash Flows'); ?></li>
    </ol>
</section>
<div class="cashFlows index" style="margin: 20px" >
                	...
                	<thead>
                	   <tr>
                    	<th><?php echo $this->Paginator->sort(__('created')); ?></th>
                            ...
                            <th><?php echo $this->Paginator->sort(__('order_id')); ?></th>
                            <th><?php echo $this->Paginator->sort(__('total')); ?></th>
                            <th class="actions col-xs-1"><?php echo __('Actions'); ?></th>
                	   </tr>
                	</thead>
                	<tbody>
                	    <tr>
                    	    <?php $base_url = array('controller' => 'Cash Flows', 'action' => 'index');?>
                            ...
                            <td><?php echo $this->Form->input('order_id', array('label' => '','default' => '','style'=>'width:100%','empty' => __('All order_id')));?></td>
                            <td><?php echo $this->Form->input('total', array('label' => '','default' => '','style'=>'width:100%','empty' => __('All total')));?></td>
						...
						<td><?php echo h($cashFlow['CashFlow']['modified']); ?>&nbsp;</td>
							<td><?php if($cashFlow['CashFlow']['active'] == 'S'){
                              echo '<span  class="btn-sm bg-green pull-right">'.__('Active').'</span>';
                           }else{
                              echo '<span  class="btn-sm bg-red pull-right">'.__('Inactive').'</span>';
                           } ?></td>
							<td>
			<?php echo $this->Html->link($cashFlow['Order']['id'], array('controller' => 'orders', 'action' => 'view', $cashFlow['Order']['id'])); ?>
		</td>
						<td><?php echo h($cashFlow['CashFlow']['total']); ?>&nbsp;</td>
							<td class="actions">
								<div class="pull-right">
									<div class="btn-group">
										...
										<ul class="dropdown-menu pull-right" role="menu">
											<li><?php echo $this->Html->link($this->Html->tag('i', __(' View'), array('class' => 'fa fa-eye fa-fw'), array('class' => 'btn btn-primary','escape' => false)), $this->Html->url(array('action' => 'view', $cashFlow['CashFlow']['id'])), array('escape' => false)); ?></li>
											<li><?php echo $this->Html->link($this->Html->tag('i', __(' Edit'), array('class' => 'fa fa-edit fa-fw'), array('class' => 'btn btn-primary','escape' => false)), $this->Html->url(array('action' => 'edit', $cashFlow['CashFlow']['id'])), array('escape' => false)); ?></li>
											<li><?php echo $this->Form->postLink($this->Html->tag('i', __(' Delete'), array('class' => 'fa fa-trash-o fa-fw')), array('action' => 'delete', $cashFlow['CashFlow']['id']),array('escape'=>false),__('Are you sure you want to delete the %s?', $cashFlow['CashFlow']['id']),array('class' => 'btn btn-mini'));?> </li>
										</ul>
									</div>
								</div>
							</td>
